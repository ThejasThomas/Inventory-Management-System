  ...
  implements IProductRepository
{
  ...
  async findByUserId(
    ...
    limit: number,
    search: string
  ): Promise<{ products: IProductEntity[]; total: number }> {
  ...
  async updateProduct(
    productId: string,
    userId: string,
    updateData: Partial<IProductEntity>
  ): Promise<IProductEntity | null> {
  ...
  async updateQuantity(name: string, qty: number) {
    await ProductModel.updateOne({ name }, { $set: { quantity: qty } });
  }
}
